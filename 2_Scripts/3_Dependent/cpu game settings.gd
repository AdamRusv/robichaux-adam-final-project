...
var playerDeck : Decks = Decks.standard
var cpuDeck : Decks = Decks.standard
enum Decks{
	standard,
	half,
	doubled,
...
func _get_deck(deckToGet : Decks) -> Deck:
	...
	match deckToGet:
		...
		Decks.custom:
			newDeck = load("res://5_Resources/0_Premade Decks/test deck.tres") #NOTE: only in itch.io version
